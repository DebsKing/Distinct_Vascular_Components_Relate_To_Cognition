...
# lm.1.rob <- lmrob(DSz ~ LVF1z +  LVF2z +  LVF3z + Sex + Education,
#                   data=mydata, 
#                   setting = "KS2014") # New setting strongly suggested by Koller and Stahel (2011). 
#                                       # Reference: Koller, M. and Stahel, W.A. (2011) Sharpening Wald-type inference in robust regression for small samples. Computational Statistics & Data Analysis 55(8), 2504-2515.
# 
...
## Previously, used 'lmrob() to handle outliers and perform robust regression. 
# However, lmrob() uses iterations and is not deterministic. In Model 3, the 
# results kept changing. Now, use rlm() function for robust regression in a 
# deterministic manner. And models will be compared not with p-valeus but with
# AIC and BIC.

#------------------------------------------------------------------------------#
### Model 1

# lm.1 <- lmrob(DSz ~ LVF1z +  LVF2z +  LVF3z + 
#                 Sex + Education, 
#               data=mydata, setting = "KS2014")
...
# lm.2 <- lmrob (DSz ~ LVF1z* scale(poly(Agez,2)) + 
...
## Previous approach
# Set seed, to avoid issues in model 3
# this is because, previously and without the set seed, model 3 showed variability in the
# results. There was no apparent reason for this, ie the regression should be determinisitic. 
# Changing drug vars to integers didn't resolve the problem.
# Setting the seed resolves the problem and gives consistent results.
# set.seed(0)
# lm.3 <- lmrob (DSz ~ 
...
# lm.4 <- lmrob(DSz ~ LVF1z*   scale(poly(Agez,2)) * Sex  + 
...
### Concise model 5
# lm.5 <- lmrob(DSz ~ LVF1z * LVF2z * LVF3z * scale(poly(Agez,2)) +
#                     Sex + Education, 
#                     data=mydata, setting = "KS2014")
### Longer model 5
# Write out terms so they are presented to anova in the same order: 
# lm.5 <- lmrob (DSz ~ LVF1z * scale(poly(Agez,2))  + 
#                  LVF2z * scale(poly(Agez,2))  + 
#                  LVF3z * scale(poly(Agez,2)) +
#                  LVF1z * LVF2z * LVF3z +
...
### Previous comparisons with Anova, using lmrob(). 
# anova(lm.1, lm.2) # 366.8 (p<0.001)
# anova(lm.2, lm.3) # 67.39, p=0.03378
# anova(lm.2, lm.4) # 8.2815, p=0.6879
# anova(lm.2, lm.5)  # 5.5922, p=0.9352. 
## Corrected p-values
#  manually put uncorrected pvalues into variable
# temp<- c(2.2e-16, 0.03378, 0.6879, 0.9352)
# And run Bonferroni corrections on this variable. 
# p.adjust(temp,  method="bonferroni")  # using holm or bonferroni
# 8.8000e-16  1.3512e-01  1.0000e+00  1.0000e+00
# p<0.001, p=0.13512, p=1.00, p=1.00
...
# The above single line anovas, give different results to
# running all models in a single anova command:
...
### Save plot
setwd("C:/Users/debs_/OneDrive - University of Cambridge/1_Vascular_Cognition/Outputs/Plots/")
ggsave(plotLVF3,filename = "LVF2-DS.png")
...
# Young:
youngdata <- subset(mydata, Age_Group == 'Young',select=c(LVF3z, DSz, Age_Group))
cor.test(youngdata$DSz,youngdata$LVF3z)  # r=0.006820874 
count(mydata$Age_Group=="Young") # 154

# Middle:
middata <- subset(mydata, Age_Group == 'Middle',select=c(LVF3z, DSz, Age_Group))
cor.test(middata$DSz,middata$LVF3z)  # r=0.0172425 
count(mydata$Age_Group=="Middle") # 307

# Old:
olddata <- subset(mydata, Age_Group == 'Old',select=c(LVF3z, DSz, Age_Group))
cor.test(olddata$DSz,olddata$LVF3z)  # r=0.1335735 
...
#------------------------------------------------------------------------------#
# ### Extxra models on 2022-02-01
# 
# # In response to Carol's comment on better understanding the relationships
# # between vascular and education variables
# 
# 
# lm.1 <- rlm(LVF1z ~ Education,data=mydata)
# lm.2 <- rlm(LVF2z ~ Education,data=mydata)
# lm.3 <- rlm(LVF3z ~ Education,data=mydata)
# 
# lm.1.age <- rlm(LVF1z ~ Education + scale(poly(Agez,2)),data=mydata)
# lm.2.age <- rlm(LVF2z ~ Education + scale(poly(Agez,2)),data=mydata)
# lm.3.age <- rlm(LVF3z ~ Education + scale(poly(Agez,2)),data=mydata)
# # Look for age effects but not age interactions, because 
# # categorical education cannot multiply by numeric age, ie 
# # education * Age
# 
# tab_model(lm.1, show.se=TRUE, digits=2)
# tab_model(lm.2, show.se=TRUE, digits=2)
# tab_model(lm.3, show.se=TRUE, digits=2)
# 
# tab_model(lm.1.age, show.se=TRUE, digits=2)
# tab_model(lm.2.age, show.se=TRUE, digits=2)
# tab_model(lm.3.age, show.se=TRUE, digits=2)
# 
# # There appears to be no interesting relationships between vascular health 
# # and education. 
