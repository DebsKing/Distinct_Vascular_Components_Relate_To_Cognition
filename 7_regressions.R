...
# sex, education, general health and medications are categorical, therefore not z-scored. 
...
# Taken from the Home Interview, question 321.
# "Would you say that for some one of your age, your own health in general is: 
# 1. excellent / 2. good / 3.fair / 4. poor... "
mydata$General_health <- as.factor(mydata$v321)

## Check that categorical variables are coded appropirately for regression 
# categorical variables should have "levels"

head(mydata$General_health)

head(mydata$Sex) 
mydata$Sex <- as.factor(mydata$Sex) # codes as a factor

mydata$AntiH_Binary <- as.factor(mydata$AntiH_Binary) # codes as a factor
mydata$drugclass2BB <- as.factor(mydata$drugclass2BB) # codes as a factor
mydata$drugclass2Diu <- as.factor(mydata$drugclass2Diu) # codes as a factor
mydata$drugclass2Stat <- as.factor(mydata$drugclass2Stat) # codes as a factor

#------------------------------------------------------------------------------#
### Notes

# In the enitre sample n=708, there are a few people missing data for education and general health
# In the n=655 sample input to regressions

sum(mydata$Education == "NaN") # 0 participants missing data in n=655
sum(mydata$General_health == "NaN") # 2 participants missing in n=655

# So the Education variable will not have a level for NaN in inear model print out,
# But the General Health variable will do so.
...
lm.1 <- rlm(DSz ~ LVF1z +  LVF2z +  LVF3z +
              General_health + 
              Education + Sex 
              -1, # include this to see all levels of "General_health", instead of intercept
            data=mydata)
...
summary(lm.1, fit.measures=T, standarised=T)

# Note that rlm() robust regression does not return r.squared

## Results Model 2 
tab_model(lm.2, show.se=TRUE, digits=2)
tab_model(lm.2, show.se=TRUE, digits=2, file="Model2_results.doc") 
summary(lm.2, fit.measures=T, standarised=T)
...
## Extract AIC fit metrics, use in table below
...
## Extract BIC fit metrics, use in table below
